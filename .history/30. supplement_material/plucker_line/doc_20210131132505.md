 ...
 * @LastEditTime: 2021-01-31 13:24:44
...
![](./picture/3.png)
过程：\
a.1. 线的在相机图像上的两个点$(s_1,e_1)$和相机本身的位置$(c_1)$可以构建出一个平面$\phi$
$$
    \begin{aligned}
    &\phi_x(x-x_{c_1})+\phi_y(y-y_{c_1})+\phi_z(y-z_{c_1})=0\\
    \rightarrow &\phi_xx+\phi_yy+\phi_zz = \phi_w
    \\
    \phi_w = \phi_xx_{c_1}+\phi_yy_{c_1}+\phi_zz_{c_1}
    \end{aligned}
    \phi = \left[
        \begin{matrix}
        \phi_x,\phi_y,\phi_z,\phi_w
        \end{matrix}
    \right]
$$
