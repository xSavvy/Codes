...
Rolling Shutter Effect 大佬们给出的讨论
   ...
   16. (SVO)仿射可以修正偏差，这里的放射是指SE3上的仿射，和光度上的放射
   17. (BASALT) 仿射可以修正偏差，这里的放射是指SE2上的仿射,也就是平面上的仿射
   18. (ORB)尽量让当前帧和更加古老的帧建立关系，来减少累计误差，这也是为什么有回环，几乎所有的算法都是Frame2Map 的匹配。ORB会有CreateNewMapPoint 的原因，说到底都是为了和更加古老的帧建立关系
   19. (SELF) patch matching + 极线搜索对于匹配有很大的潜力，可以好好探索一下的。
